...
class HotelBookingParser:
        ...
        try:
            ...
            if spreadsheet_id:
                try:
                    ...
                    result["spreadsheet_title"] = spreadsheet.get('properties', {}).get('title', 'Unknown')
                    result["message"] = f"Successfully connected. Access to spreadsheet '{result['spreadsheet_title']}' confirmed."
                    
                except HttpError as e:
                    result["spreadsheet_accessible"] = False
                    error_reason = e.error_details[0].get('reason', 'Unknown error') if e.error_details else str(e)
                    
                    if e.resp.status == 404:
                        result["error"] = f"Spreadsheet not found (ID: {spreadsheet_id})"
                        result["message"] = "Authenticated, but spreadsheet not found. Check the spreadsheet ID."
                    elif e.resp.status == 403:
                        result["error"] = "Permission denied"
                        result["message"] = "Authenticated, but no access to this spreadsheet. Make sure the service account has access."
                    else:
                        result["error"] = f"HTTP {e.resp.status}: {error_reason}"
                        result["message"] = f"Authenticated, but error accessing spreadsheet: {error_reason}"
                    
                except Exception as e:
                    result["spreadsheet_accessible"] = False
                    result["error"] = str(e)
                    result["message"] = f"Authenticated, but error accessing spreadsheet: {str(e)}"
    ...
    def _parse_dates_from_start_cell(
        ...
        try:
            ...
            if start_date_str:
                ...
                if not start_date:
                    logger.warning(f"Could not parse start date from env: {start_date_str}")
            
            # Option 2: Try to read from the cell itself
            if not start_date:
                if start_row_idx < len(data) and start_col_idx < len(data[start_row_idx]):
                    cell_value = data[start_row_idx][start_col_idx]
                    if cell_value:
                        cell_str = str(cell_value).strip()
                        start_date = self._parse_date(cell_str)
                        if start_date:
                            logger.info(f"Parsed start date from cell {start_cell}: {start_date.date()}")
            
            if not start_date:
                logger.error(f"Could not determine start date from cell {start_cell} or env. Please provide DATE_START in env or ensure the cell contains a date.")
                return None, {}
            ...
            logger.info(f"Starting date parsing from cell {start_cell} with date {current_date}")
            ...
            if date_map:
                logger.info(f"Successfully parsed {len(date_map)} dates starting from {start_cell}")
    ...
    def _find_header_row(self, data: List[List]) -> Tuple[Optional[int], Dict[datetime.date, int]]:
    ...
    def load_calendar(
        ...
        try:
            ...
            if final_date_start_cell:
                ...
                if final_date_start:
                    logger.info(f"Using start date from parameter/env: {final_date_start}")
    ...
    def get_available_categories(self, check_in: datetime, check_out: datetime) -> List[str]:
    ...
    def get_calendar_info(self) -> Dict:
        ...
        return {
            ...
            "header_row": self.header_row_index + 1 if self.header_row_index is not None else None,
            "data_start_row": self.data_start_row + 1 if self.data_start_row is not None else None,
            "sample_dates": [str(d) for d in dates[:10]] if len(dates) > 10 else [str(d) for d in dates]
        }
