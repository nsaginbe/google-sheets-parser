"""FastAPI application for hotel booking parser service"""
...
from app.models import (
    ...
    LoadCalendarRequest, LoadCalendarResponse, CalendarInfo,
...
async def root():
    """Root endpoint"""
    return {
        "message": "Hotel Booking Parser API",
        "version": "1.0.0",
        "status": "running"
    }
...
async def get_available_rooms(request: AvailabilityRequest):
    ...
    - **category_filter**: Optional category filter (e.g., "Deluxe")
    """
    if parser is None:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
...
@app.post("/categories/available", response_model=CategoriesResponse, tags=["Categories"])
